...
#define BTRFS_CBS_ROOT_OBJECTID 40ULL
#define BTRFS_CBS_ITEM_KEY 45ULL


#define BTRFS_CBS_EXTENT_ITEM_KEY 52ULL
/* CBS tree(experimental) */
#define BTRFS_CBS_TREE_OBJECTID 102ULL
...
struct btrfs_cbs_hash {
	...
	u8  type;
	int compression;
	u8 hash[]; /* this is a hash field  */

} __attribute__ ((__packed__));

/******/
struct btrfs_cbs_data {
	...
	u8 type;
	u16 other_encoding;   
	u8 hash[];  /* initial 3 parts of hash */
...
struct btrfs_fs_info {
	...
	/*
	 * Content-based storage mode root and extent size
	 */
...
/******/
BTRFS_SETGET_FUNCS(cbs_len, struct btrfs_cbs_data, len, 64);
BTRFS_SETGET_FUNCS(cbs_compression, struct btrfs_cbs_data, compression, 8);
BTRFS_SETGET_FUNCS(cbs_encryption, struct btrfs_cbs_data, encryption, 8);
BTRFS_SETGET_FUNCS(cbs_other_encoding, struct btrfs_cbs_data, other_encoding, 16);
BTRFS_SETGET_FUNCS(cbs_type, struct btrfs_cbs_data, type, 8);

BTRFS_SETGET_FUNCS(dedup_len, struct btrfs_cbs_data, len, 64);
BTRFS_SETGET_FUNCS(dedup_compression, struct btrfs_cbs_data, compression, 8);
BTRFS_SETGET_FUNCS(dedup_encryption, struct btrfs_cbs_data, encryption, 8);
BTRFS_SETGET_FUNCS(dedup_other_encoding, struct btrfs_cbs_data, other_encoding, 16);
BTRFS_SETGET_FUNCS(dedup_type, struct btrfs_cbs_data, type, 8);
...
int btrfs_add_ordered_extent_dedup(struct inode *inode, u64 file_offset,
				   ...
				   int dedup, struct btrfs_cbs_hash *hash,
				   int compress_type);

/******/
int __btrfs_add_ordered_extent(struct inode *inode, u64 file_offset,
				      u64 start, u64 len, u64 disk_len,
				      int type, int dio, int compress_type,
				      int cbs, struct btrfs_cbs_hash *hash);


/******/
int __btrfs_free_reserved_extent(struct btrfs_root *root, u64 start, u64 len, int pin, int delalloc);
